...
class Input2SQL(object):
    def main(self):
        ...
        #df.rename(columns={'SKU ID': 'sk', 'Search Keyword': 'search_key'}, inplace=True)
        #engine = create_engine("mysql+pymysql://{user}:{pw}@localhost/{db}?charset=utf8".format(user="root", pw="root", db="urlqueue"))
        #df.to_sql('ebay_crawl', con = engine, if_exists = 'append', chunksize = 1000, index=False)
