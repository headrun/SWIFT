...
class EbaySpider(scrapy.Spider):
    ...
    def __del__(self):
        df = pd.DataFrame(self.final_result, columns=['SKU ID', 'Search Key', 'ItemListingURL', 'Title', 'Condition', 'End Time', 'Price', 'Image URL', 'Location', 'Item ID', 'Category', 'Timestamp'])
    ...
    def start_requests(self):
        ...
        for key in key_words:
            ...
            yield Request(url, callback=self.parse, headers = self.headers, meta = {'sku' : sku, 'search_key':search_key, 'end_time': end_time})
    ...
    def parsedata(self, response):
        ...
        self.final_result.append([sku, search_key, prod_url, title, condition, end_date, price, img_url, location, item_id, category, timestamp])
