...
listing = 'https://www.ebay.com/sch/i.html'
conn = MySQLdb.connect(host='localhost', user='root', passwd= 'password@123', db="conversion", charset='utf8mb4', use_unicode=True)
cursor = conn.cursor()
cursor.execute('select sk, search_key, end_time from ebay_crawl limit 15')
...
for key in key_words:
    ...
    res = Selector(text=requests.get(listing, headers=headers, params=params).text)
    ...
    for url in item_urls:
        ...
        final_result.append([sku, search_key, prod_url, title, condition, end_date, price, img_url, location, item_id, category, timestamp])
