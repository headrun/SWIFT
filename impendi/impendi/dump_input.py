...
class Input2SQL():
    def main(self):
        ...
        df = df[['SKU ID', 'Search Keyword', 'End Time']]
        values = [tuple(item) for item in df._values]
        query = 'insert into ebay_crawl (sk, search_key, end_time, created_at, modified_at) values(%s, %s, %s, now(), now()) on duplicate key update modified_at = now()'
