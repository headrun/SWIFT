...
#import configUtils
import os
from sports_spiders.vtv_utils import vtv_pickle, vtv_unpickle, VTV_SERVER_DIR
...
class SportsdbSetup:
    ...
    def __init__(self, item, cursor, spider_class='', gids_file='', hash_conf=None):
        ...
        self.get_sport_ids()
        self.out_file = open('%s/missed_participants_sks_%s' %
                             (FILE_PATH, self.cur_date), 'a+')
        self.missing_tou_list = {}
        self.mis_file = open('%s/missing_tournaments_list_%s' %
                             (STATS_DIR, self.cur_date), 'a+')

    def clean(self):
        release_logger()

    # check for sk existance in source_keys table
    def check_source_keys(self, source_key, participant_type='', source=''):
        try:
            ...
            if not res or None in res:
                self.cursor.execute(self.hash_conf['SK_CHECK_QUERY'] % (
                    participant, source_key, source))  # check entity_id
    ...
    def populate_stats(self, stats):
        for result_type, values in stats.items():
            for team, result_values in values.items():
                participant_id = self.get_participant_id(team, 'participant')
                if not participant_id:
                    continue
                counter = 1
                for result_value in result_values:
                    player_id = self.get_participant_id(
                        result_value[0], 'participant')
                    try:
                    ...
                    if result_type.endswith('_'):
                        ...
                        values_1 = (self.entity_id, participant_id,
                                    result_id, player_id, player_id)
                        values_2 = (self.entity_id, participant_id,
                                    res_value, result_value[1], result_value[1])
                        self.cursor.execute(
                            self.hash_conf['INSERT_SCORES'], values_1)
                        self.cursor.execute(
                            self.hash_conf['INSERT_SCORES'], values_2)
                        counter += 1
    ...
    def get_pl_game(self, sport_id):
        ...
        query = 'select title from sports_types where id=%s' % (sport_id)
    ...
    def get_participant_id(self, participant, participant_type=''):
        existance, result = self.check_source_keys(
            participant, participant_type)
        if existance == 1:
    ...
    def create_game_id(self):
        query = "select auto_increment from information_schema.TABLES where TABLE_NAME='sports_games' and TABLE_SCHEMA='%s'" % self.hash_conf[
            'DB_NAME']
        self.cursor.execute(query)
    ...
    def get_tournament_id(self):
        if self.game:
            self.sport_id = self.sports_id_dict.get(self.sport_name, '')
            if not self.sport_id:
                self.sport_id = self.sports_id_dict.get(self.game, '')
            self.cursor.execute('SELECT id FROM sports_tournaments WHERE title=\"%s\" and sport_id=\"%s\"' % (
                self.tournament, self.sport_id))
        else:
            self.cursor.execute(
                'SELECT id FROM sports_tournaments WHERE title=\"%s\"' % (self.tournament))
        data = self.cursor.fetchone()
        if not data:
            self.cursor.execute(
                'SELECT id FROM sports_tournaments WHERE title=\"%s\"' % (self.tournament))
        ...
        if data:
            ...
            tid = 0
            title = "Missing Tournament Name: " + self.tournament
            self.missing_tou_list.setdefault(
                title, []).append([self.game, self.tournament])
            self.mis_file.write('%s\n' % self.missing_tou_list)
        return tid

    def get_group_id(self, title):
        self.cursor.execute(
            'select id from sports_tournaments_groups where group_name = \"%s\"' % (title))
        data = self.cursor.fetchone()
        if data:
            tid = data[0]
        else:
            self.cursor.execute(
                'select id from sports_tournaments_groups where aka = \"%s\"' % (title))
            data = self.cursor.fetchone()
            if data:
                tid = data[0]
            else:
    ...
    def update_game_locations(self, loc, stadium):
        ...
        if stadium and country:
            stadium_query = 'select S.id, S.location_id from sports_stadiums S, sports_locations L where S.location_id=L.id and S.title=%s and L.country=%s'
            std_values = (stadium, country)
            self.cursor.execute(stadium_query, std_values)
            std_data = self.cursor.fetchone()
            if not std_data:
                stadium_title = '%' + stadium + '%'
                query = 'select S.id, S.location_id from sports_stadiums S, sports_locations L where S.location_id=L.id and S.title like %s and L.country=%s'
                std_values = (stadium_title, country)
                self.cursor.execute(query, std_values)
                std_data = self.cursor.fetchone()
                if not std_data:
                    stadium_query = 'select S.id, S.location_id from sports_stadiums S, sports_locations L where S.location_id=L.id and S.aka like %s and L.country=%s'
                    std_values = (stadium_title, country)
                    self.cursor.execute(stadium_query, std_values)
                    std_data = self.cursor.fetchone()
            if std_data:
        ...
        if (not loc_id or loc_id == 0) and (state or city or country):
            ...
            if (not loc_id or loc_id == 0):
                self.cursor.execute(self.hash_conf['LOC_INSERTION'], (
                    continent, country, state, city, street, zipcode, latlong))
                ...
                values = (continent, country, state,
                          city, street, zipcode, latlong)
                self.cursor.execute(query, values)
                loc_id = self.cursor.fetchone()[0]

        if (not std_id or std_id == 0) and stadium and stadium.lower() != "null":
            self.cursor.execute(
                "select auto_increment from information_schema.TABLES where TABLE_NAME='sports_stadiums' and TABLE_SCHEMA='SPORTSDB'")
            count = self.cursor.fetchone()
            stadium_gid = 'STAD%s' % str(count[0])
            self.cursor.execute(
                self.hash_conf['STD_INSERTION'], (stadium_gid, stadium, loc_id, loc_id))
    ...
    def get_eventid(self):
        ...
        if not event_id:
            self.cursor.execute(
                'SELECT id FROM sports_tournaments WHERE title=\"%s\"' % (event_name))
            data = self.cursor.fetchone()
            if data:
                event_id = data[0]
    ...
    def participant_title_check(self, title, callsign):
        ...
        if not self.sport_id:
            self.sport_id = self.sports_id_dict.get(self.game, '')
        self.cursor.execute(self.hash_conf['PARTICIPANT_TITLE_AKA_CHECK'] % (
            title, callsign, self.sport_id, self.affiliation))
    ...
    def update_participants(self):
        ...
        for pt in self.participants:
            ...
            pid = self.check_participants(callsign)
            if pid and len(pid) == 1:
                ...
                if pid and len(pid) == 1:
                    pid = str(pid[0])
                    self.cursor.execute(
                        self.hash_conf['PID_EXISTANCE_IN_SKS'], ('participant', pid))
                    pres = self.cursor.fetchone()
                    if not pres:
                        self.cursor.execute(
                            self.hash_conf['INSERT_SKS'], (pid, 'participant', self.affiliation.upper(), callsign))
                    if isinstance(self.participants, dict):
    ...
    def update_game_scores(self, pt_ids):
        ...
        if isinstance(self.result, dict):
            for pt_sk, values in self.result.items():
                if pt_sk == '0':
                    pt_id = '0'
                else:
                        ...
                        if 'winner' in key and isinstance(value, list):
                            for v in value:
                                ...
                                if winner_id:
                                    ...
                                    self.cursor.execute(
                                        self.hash_conf['INSERT_SCORES'], values)
                        elif value != '':
                            if 'jersey' in key:
                                ...
                                if winner_id:
                                    ...
                                    if self.game in eval(self.hash_conf['TOU_WINNER_LIST']) and self.season is not None:
                                        ...
                                        if tou_id != 0 and not 'pole_winner' in key:
                                            values = (
                                                tou_id, self.season, key, '', value)
                                            self.cursor.execute(
                                                self.hash_conf['INSERT_TOU_RESULTS'], values)
                            values = (self.entity_id, str(
                                pt_id), key, value, value)
                            self.cursor.execute(
                                self.hash_conf['INSERT_SCORES'], values)
            return
        ...
        for pt in pt_ids:
            ...
            if pscore or self.source == 'mlb' or self.source == 'MILB':
                ...
                for i in range(len(goals)):
                    ...
                    if not goaler or not goal_time:
                        continue
                    rt = 'G%s' % (str(i+1))
                    ...
                    rt = 'GT%s' % (str(i+1))
                    values = (self.entity_id, str(pid),
                              rt, goal_time, goal_time)
                    self.cursor.execute(
                        self.hash_conf['INSERT_SCORES'], values)
    ...
    def check_participants(self, callsign):
        ...
        if srcs:
            ...
            if len(res) > 1:
                for res_ in res:
                    ...
                    self.cursor.execute(
                        self.hash_conf['PID_CHECK_TYPE_QUERY'], values)
        ...
        if not res:
            if self.reference_url is None:
                ...
                record = '<>'.join(
                    [self.game, self.source, callsign, self.reference_url, self.tournament])
    ...
    def handle_richdata_videos(self):
        ...
        for video in videos:
            ...
            if 'high' in list(video.keys()) and 'low' in list(video.keys()):
                if video.get('high', ''):
            ...
            if sk and title:
                ...
                video_val = (sk, ref, high_link, low_link, high_mime, low_mime, clean_data(
                    title), clean_data(desc), img, other_links)
    ...
    def get_game_participants(self):
        self.cursor.execute(
            self.hash_conf['PARTICIPANT_IDS'], (self.entity_id,))
    ...
    def update_unknown_source(self):
        ...
        for k, v in eval(self.hash_conf['AFF_SRC']).items():
    ...
    def populate_series_name(self):
        ...
        if not series_id:
            values = (series, location_id,
                      series_season_start, series_season_end)
        ...
        for value, key in pt_ids:
            if int(key) != 0:
                ...
                if not 'tbd' in pt_name.lower():
                    self.cursor.execute(
                        self.hash_conf['INSERT_TOU_PARTICIPANTS'], (value, tid))

    def create_new_record(self):
        if self.game not in eval(self.hash_conf['NON_PARTICIPANT_GAMES'])\
            and not self.participants or\
            (isinstance(self.participants, list) and
                self.participants[0].get('callsign')
                == self.participants[1].get('callsign')):
            return
        ...
        event_id = 0
        if self.event:
            event_id = self.get_eventid()

        if event_id == 0:
            event_id = tid

        if event_id and self.source in eval(self.hash_conf['GRAND_SLAM']) and 'constructed' in self.game_status:
            query = self.hash_conf['TOURNAMENT_START'] % event_id
            self.cursor.execute(query)
            event_date = self.cursor.fetchone()[0]
            if event_date > self.cur_date:
                game_date = event_date.strftime('%Y-%m-%d')
                self.game_datetime = game_date + ' 12:00:00'
            else:
                game_date = (self.cur_date + timedelta(days=1)
                             ).strftime('%Y-%m-%d')
                self.game_datetime = game_date + ' 12:00:00'
            self.game_status = 'scheduled'

            self.source_key = self.source_key.rsplit(
                '_', 3)[0] + '_' + game_date.replace('-', '_')
        ...
        if tid and (len(pt_ids) == 2 or self.participant_type == 'player') or \
            ...
            self.entity_id, game_id = self.create_game_id()
            ref_url = ''
            ...
            game_values = (self.entity_id, game_id, self.game_datetime, self.sport_id,
                           rd[0], tid, self.game_status, rd[1], rd[2], rd[3], ref_url, event_id, loc_id, std_id, self.time_unknown, self.tz_info)
            ...
            for v, k in pt_ids:
                if self.game in eval(self.hash_conf['UPDATE_GROUP_PARTICIPANTS_GAMES_LIST']):
                    self.cursor.execute(
                        self.hash_conf['GAME_GROUP_PTS'], (self.entity_id, v, k))
                else:
                    self.cursor.execute(
                        self.hash_conf['GAME_PTS'], (self.entity_id, v, k))

            if len(pt_ids) == 2 and self.game_status == 'completed':
            ...
            if self.affiliation.lower() in eval(self.hash_conf['ALLOWED_AFFILIATION']):
                if 'tbd_title' in self.rich_data and self.rich_data.get('tbd_title', ''):
                    ...
                    if pt_id:
                        values = (self.entity_id, str(
                            pt_id[0]), 'tbd_title', value, value)
                        self.cursor.execute(
                            self.hash_conf['INSERT_SCORES'], values)

                source = self.affiliation.upper()
            if self.affiliation not in self.hash_conf['ALLOWED_AFFILIATION']:
                source = self.source

            self.cursor.execute(
                self.hash_conf['INSERT_SKS'], (self.entity_id, 'game', self.source, self.source_key))
    ...
    def update_record(self):
        ...
        if self.game in eval(self.hash_conf['UPDATE_PARTICIPANTS_GAMES_LIST']):
            db_ids = set([tuple(sorted((str(i[0]), str(i[1]))))
                          for i in pt_ids])
            db_ids_ = set([tuple(sorted((str(i[0]), str((i[1]))))) for i in pt_ids])
            pts = self.update_participants()
            if self.game == "basketball":
                if pts != db_ids_:
                    remove_pids = tuple(db_ids_)
                    for remove_pid in remove_pids:
                        self.cursor.execute(
                            self.hash_conf['REMOVE_GAME_PAT'], (remove_pid[1], self.entity_id))
                    pids = pts  # tuple(id_diff)
                    for pid in pids:
                        if self.game in eval(self.hash_conf['UPDATE_GROUP_PARTICIPANTS_GAMES_LIST']):
                            self.cursor.execute(
                                self.hash_conf['GAME_GROUP_PTS'], (self.entity_id, pid[1], pid[0]))
                        else:
                            self.cursor.execute(
                                self.hash_conf['GAME_PTS'], (self.entity_id, pid[1], pid[0]))
            ...
            try:
                ...
                return
            if len(db_ids) == 1 or not db_ids:
                remove_pids = tuple(db_ids)
                for remove_pid in remove_pids:
            ...
            elif len(db_ids) != len(pts) or (sorted(db_ids)[0][1] not in sorted(pts)[0][1]) or (sorted(db_ids)[1][1] not in sorted(pts)[1][1]):
                # if id_diff:
                remove_pids = tuple(db_ids)
                for remove_pid in remove_pids:
                    #self.cursor.execute(self.hash_conf['REMOVE_GAME_RES_PAT'], (remove_pid[1], self.entity_id))
        ...
        if len(pt_ids) <= 1:
            ...
            if isinstance(pts, set):
                for pt in pts:
                    self.cursor.execute(
                        self.hash_conf['GAME_PTS'], (self.entity_id, pt[1], pt[0]))
                    pt_ids.append((pt[1], pt[0]))
            else:
                for k, v in pts.items():
                    self.cursor.execute(
        ...
        if self.game_status in ['completed', 'ongoing', 'retired'] \
        ...
        if not event_id or str(event_id) == '197' or str(event_id) == '977':
            ...
            values = (self.game_status, event_id,
                      self.time_unknown, self.entity_id)
            # UPDATE GAME STATUS
            self.cursor.execute(self.hash_conf['UPDATE_RECORD'], values)
        sport_id = self.sports_id_dict.get(self.sport_name, '')
        if not sport_id:
            sport_id = self.sports_id_dict.get(self.game, '')
        ...
        if game_note and game_note is not None:
            self.cursor.execute(
                self.hash_conf['UPDATE_GAME_NOTE'], (game_note, self.entity_id))
        ...
        if radio_info is not None and radio_info != '':
            self.cursor.execute(
                self.hash_conf['UPDATE_RADIO'], (radio_info, self.entity_id))
        if channel_info is not None and channel_info != '':
            self.cursor.execute(
                self.hash_conf['UPDATE_CHANNEL'], (channel_info, self.entity_id))

        if self.game_status == "scheduled":
            if self.game_datetime:
                values = (self.game_datetime,
                          self.time_unknown, self.entity_id)
        ...
        if self.game_status == 'completed' and self.source == 'espn_rugby':
            if self.game_datetime:
                values = (self.game_datetime,
                          self.time_unknown, self.entity_id)
                self.cursor.execute(self.hash_conf['UPDATE_GAMETIME'], values)

        if self.game_status == 'completed' and self.source == 'mlb':
            if self.game_datetime:
                values = (self.game_datetime,
                          self.time_unknown, self.entity_id)
                self.cursor.execute(self.hash_conf['UPDATE_GAMETIME'], values)
        ...
        if self.game_status == 'cancelled' and self.source == 'mlb':
            if self.game_datetime:
                values = (self.game_datetime,
                          self.time_unknown, self.entity_id)
                self.cursor.execute(self.hash_conf['UPDATE_GAMETIME'], values)

        if self.game_status == 'completed' and self.source == 'NFL':
            if self.game_datetime:
                values = (self.game_datetime,
                          self.time_unknown, self.entity_id)
                self.cursor.execute(self.hash_conf['UPDATE_GAMETIME'], values)

        if self.game_status == 'scheduled' and self.source == 'espn_ncaa-ncf':
            if self.tournament:
                gtsel_query = 'select id from sports_tournaments where title = "%s" and sport_id = %s'
                values = (self.tournament, str(self.sport_id))
                self.cursor.execute(gtsel_query % values)
                _id = self.cursor.fetchone()
                gtupdate_query = 'UPDATE sports_games SET tournament_id=%s where id=%s'
                gtup_values = (str(_id[0]), self.entity_id)
                self.cursor.execute(gtupdate_query % gtup_values)
    ...
    def populate_tournament_standings(self):
        ...
        if not tournament_id:
            return
        for team_sk, values in self.result.items():
            ...
            if not team_id:
                ...
                if not team_id:
                    continue
            for key, value in values.items():
                values = (tournament_id, team_id, self.season,
                          self.id_type, 'standings', key, value, value)
                self.cursor.execute(
                    self.hash_conf['TOU_STANDINGS_RESULTS'], values)
                values = (team_id, tournament_id, self.season, self.season)
                #self.cursor.execute(self.hash_conf['INSERT_TOU_PARTICIPANTS'], values)
                tou_values = (tournament_id)
                self.cursor.execute(
                    self.hash_conf['TOU_STD_ORDER'] % tou_values)
                tou_data = self.cursor.fetchall()
                if tou_data:
                    result_value = tou_data[0][-1]
                    values = (tournament_id, 0, self.season,
                              self.id_type, 'standings_order', '', result_value)
                    self.cursor.execute(
                        self.hash_conf['TOU_STDORDER_RESULT'], values)
        ...
        for team_sk, values in self.result.items():
            ...
            for key, value in values.items():
                ...
                values = (group_id, team_id, self.season,
                          group_id, team_id, self.season)
                self.cursor.execute(
                    self.hash_conf['INSERT_GRP_PARTICIPANTS'], values)

                group_values = (group_id)
                self.cursor.execute(
                    self.hash_conf['GR_STD_ORDER'] % group_values)
                gr_data = self.cursor.fetchall()
                if gr_data:
                    result_value = gr_data[0][-1]
                    values = (group_id, 0, 'standings_order',
                              result_value, self.season)
                    self.cursor.execute(
                        self.hash_conf['GR_STDORDER_RESULT'], values)

    def update_rosters(self):
        for team_sk, participants_data in self.participants.items():
            team_id = self.check_participants(str(team_sk))
            if not team_id or len(team_id) > 1:
                continue
            self.source = self.item.get('source', '')
            # if self.source == "espn_cricket" or self.source == "nrl_rugby" or self.source=="NHL":
            if self.source == "nrl_rugby":
                self.cursor.execute(
                    self.hash_conf['INACTIVE_HEAD_ROSTER'] % str(team_id[0]))
            else:
                self.cursor.execute(
                    self.hash_conf['INACTIVE_ROSTER'] % (str(team_id[0])))
            for player_sk, player_data in participants_data.items():
                player_id = self.check_participants(str(player_sk))
                if not player_id:
                    print(player_sk)
                    continue
                if player_id[0] == '271458':
                    player_id[0] = '66313'
                elif player_id[0] == '276773':
                    player_id[0] = '274516'
                elif player_id[0] == '276801':
                    player_id[0] = '274573'
                elif player_id[0] == '276778':
                    player_id[0] = '274571'
                elif player_id[0] == '276793':
                    continue
                player_role = player_data.get('player_role', '')
                player_number = player_data.get('player_number', None)
                if player_number == '':
                    player_number = None
                ...
                if status == 'active' and self.source.lower() in source_template:
                    default_template = self.hash_conf[source_template[self.source.lower(
                    )][1]]
                    self.cursor.execute(
                        self.hash_conf['PARTICIPANT_TITLE'] % player_id[0])
                    title, sport_id, reference_url = self.cursor.fetchone()
                    game = self.get_pl_game(sport_id)
                    self.cursor.execute(
                        self.hash_conf['PARTICIPANT_TITLE'] % team_id[0])
                    team_details = self.cursor.fetchone()
                    field_text = default_template % (
                        title, game, player_role, source_template[self.source.lower()][0], team_details[0], reference_url)
                    values = (
                        language, player_id[0], 'participant', field_type, field_text, field_text.strip())
                    self.cursor.execute(
                        self.hash_conf['INSERT_DESCRIPTION'], values)

                if not player_role:
                    values = (team_id[0], player_id[0], player_role, player_number, status, '',
                              self.season, player_number, status, self.season)
                    self.cursor.execute(
                        self.hash_conf['INSERT_ROSTER_WITHOUT_ROLE'], values)
                else:
                    values = (team_id[0], player_id[0], player_role, player_number, status, '', self.season,
                              player_role, player_number, status, self.season)
                    self.cursor.execute(
                        self.hash_conf['INSERT_ROSTER'], values)
    ...
    def process_record(self):
        ...
        if self.spider_class:
            self.stats_file = os.path.join(
                STATS_DIR, self.spider_class + '.pickle')
        else:
            self.stats_file = ''
        ...
        entity_id = 0
        if source_key and self.source in ['australianopen_tennis']:
            source_key = source_key.rsplit('_', 3)[0]
            query = self.hash_conf['GAME_EXISTANCE_PATTERN'] % (
                self.source, source_key)
            self.cursor.execute(query)
            entity_id = self.cursor.fetchall()
            if len(entity_id) > 1:
                return
            ...
            if not entity_id:
                query = self.hash_conf['GAME_EXISTANCE_CHECK'] % (
                    source_key, source)
                self.cursor.execute(query)
