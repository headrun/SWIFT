...
################################################################################
#$Id: sports_validations.py,v 1.1 2016/03/23 07:19:22 headrun Exp $
################################################################################
...
class Validator:
    def __init__(self, stats_file):
        ...
        self.conn          = MySQLdb.connect(host="10.28.218.81", db="SPORTSDB", user="veveo", passwd="veveo123")
    ...
    def check_grandslam(self):
        ...
        if "FrenchOpenOpen" in self.stats_file:
            tournament_domain = "French Open"
        if "UsopenTennis" in self.stats_file:
            tournament_domain = "U.S. Open (Tennis)"
        query = 'select id, title from sports_tournaments where title like "%%%s%%" and type = "tournament" and sport_id = "5"'
    ...
    def check_golf(self):
        ...
        query = "select distinct(tournament_id), count(*) from sports_games \
                ...
                year(game_datetime) = year(now()) and sport_id = '8' and \
    ...
    def check_nhl_nfl_mlb(self):
        ...
        for source in src:
            ...
            for entity_id in entity_ids:
                ...
                query = 'select count(id) from sports_games where id=%s and sport_id="%s" and status="scheduled"'
                values = (game_id, game)
    ...
    def check_motogp(self):
        query = "select distinct(tournament_id), count(*) from sports_games \
                where status in ('scheduled', 'completed') and \
                year(game_datetime) > year(now()) and sport_id = '13' \
    ...
    def run_main(self):
        ...
        if 'UsopenTennis' in self.stats_file or 'AusOpenTennis' in self.stats_file or 'FrenchOpen' in self.stats_file or 'Wimbledon' in self.stats_file:
