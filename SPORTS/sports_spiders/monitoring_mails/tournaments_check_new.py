...
def main():
    ...
    text += '<br /><br /><b>Tournaments having scheduled games</b><br /><table border="1" style="border-collapse:collapse;" cellpadding="3px" cellspacing="3px"><tr><th>Id</th><th>Title</th><th>Season_Start</th><th>Season_End</th></tr>'
    for tournament_date in tournament_dates:
        if tournament_date[0] in tournament_id:
            cursor.execute('select max(game_datetime) from sports_games where tournament_id = %s and status = "scheduled"' %tournament_date[0])
            data = cursor.fetchone()
            if data[0].date() < tournament_date[2].date() or data[0].date() > tournament_date[3].date():
                wrong_dates.add(tournament_date)
            cursor.execute('select min(game_datetime) from sports_games where tournament_id = %s and status = "scheduled"' %tournament_date[0])
            data = cursor.fetchone()
            if data[0].date() < tournament_date[2].date() or data[0].date() > tournament_date[3].date():
                wrong_dates.add(tournament_date)
        elif tournament_date[0] in event_ids:
            cursor.execute('select max(game_datetime) from sports_games where event_id = %s and status = "scheduled"' %tournament_date[0])
            data = cursor.fetchone()
            if data[0].date() < tournament_date[2].date() or data[0].date() > tournament_date[3].date():
                wrong_dates.add(tournament_date)
            cursor.execute('select min(game_datetime) from sports_games where event_id = %s and status = "scheduled"' %tournament_date[0])
            data = cursor.fetchone()
            if data[0].date() < tournament_date[2].date() or data[0].date() > tournament_date[3].date():
                wrong_dates.add(tournament_date)

        if tournament_date[3] < datetime.datetime.now(): 
            text += "<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>" %(tournament_date[0], tournament_date[1], tournament_date[2].strftime("%Y-%m-%d"), tournament_date[3].strftime("%Y-%m-%d"))
    text += "</table>"
