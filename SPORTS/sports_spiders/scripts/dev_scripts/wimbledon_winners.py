...
TOU_RESULTS = "insert ignore into sports_tournaments_results (tournament_id, participant_id, season, result_type, result_sub_type, result_value) values(%s, '0', '2015', '%s', '%s', %s) ON DUPLICATE KEY UPDATE result_sub_type='%s', result_type = '%s'"


def main():

    cursor = MySQLdb.connect(host='10.4.18.183', user="root", db="SPORTSDB").cursor()
    query  = "select tournament_id, final_event_id from sports_tournaments_finals where final_event_id in (select id from sports_tournaments where title like '%wimbledon%' and title like '% final%')"
    ...
    for event in data:
        ...
        if event_id :
            ...
            winner_group = str(group_name[0]).lower().replace("'s", '').replace('wimbledon ', '')
