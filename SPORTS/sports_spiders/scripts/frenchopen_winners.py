...
def main():
    ...
    query  = "select tournament_id, final_event_id from sports_tournaments_finals where final_event_id in (select id from sports_tournaments where title like '%french open%' and title like '% final%')"
    ...
    for event in data:
        ...
        if event_id :
            ...
            winner_group = str(group_name[0]).lower().replace("'s", '').replace('french open ', '')
		...
		cursor.execute(TOU_RESULTS %('267', 'winner', winner_group.lower(), result_value[0][0], winner_group.lower(), 'winner'))
                cursor.execute(TOU_RESULTS %('267', 'winner2', winner_group.lower(), result_value[1][0], winner_group.lower(), 'winner2'))
