...
TOU_RESULTS = "insert ignore into sports_tournaments_results (tournament_id, participant_id, season, result_type, result_sub_type, result_value) values(%s, '0', '2018', %s, %s, %s) ON DUPLICATE KEY UPDATE result_sub_type=%s, result_type = %s"


def main():
    cursor = MySQLdb.connect(host='10.28.218.81', user="veveo", passwd="veveo123", db="SPORTSDB").cursor()
    query  = "select tournament_id, event_id from sports_tournaments_events where tournament_id in (select id from sports_tournaments where subtype='winter' and type='event' and affiliation='IOC')"
          ...
          if event_id :
              ...
              for result_val in result_value:
                  ...
                  if winner_group:
                      ...
                      cursor.execute(TOU_RESULTS ,('1281', result_type, winner_group, result_val[0], winner_group, result_type))
                      if final_event_id:
