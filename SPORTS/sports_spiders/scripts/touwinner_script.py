...
def main():
    ...
    query  = "select tournament_id, final_event_id from sports_tournaments_finals where final_event_id in (select id from sports_tournaments where title like '%% final%%' and sport_id not in ('5', '3', '1'))"
    #query  = "select tournament_id, final_event_id from sports_tournaments_finals where final_event_id in (select id from sports_tournaments where title like '%% final%%' and sport_id ='7')"
    ...
    for event in data:
        tou_id = event[0]
        event_id = event[1]
        game_query = 'select id, YEAR(game_datetime), status, sport_id from sports_games where event_id = %s and YEAR(game_datetime) = year(now()) and status = "completed" and sport_id=7'
        ...
        if event_id :
            ...
            if status=="completed" and str(year) == _date:
