...
class BasketballTeams():
		...
		sel_query = '''select id, record from test_cases where record like "% basketball#<>#team#<>#SPORTS#<>#TOU313#<>#%" and record not like "%men's basketball%"'''
		self.cursor.execute(sel_query)
		data = self.cursor.fetchall()
		for data_ in data:
			id_ = str(data_[0])
			title = str(data_[1])
			title = title.replace(" basketball", " men's basketball")
