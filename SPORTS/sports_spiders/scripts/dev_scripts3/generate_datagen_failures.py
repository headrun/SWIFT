...
################################################################################
#$Id: generate_datagen_failures.py,v 1.1 2016/03/23 07:10:05 headrun Exp $
################################################################################
...
class DatagenFailures:
    ...
    def __init__(self):
        ...
        self.db_ip          = "10.4.18.183"
        self.logger         = initialize_timed_rotating_logger('datagen_errors.log')
        self.conn           = MySQLdb.connect(user="root", db="SPORTSDB", host=self.db_ip)
    ...
    def send_mail(self, text):
        ...
        receivers  = ['jhansi@headrun.com']
        #receivers.extend(['raman.arunachalam@rovicorp.com', 'harshad.inarkar@rovicorp.com', 'Arun.Nagarajan@rovicorp.com', 'Jagadeswar.Mettupalli@rovicorp.com'])
    ...
    def copy_log_file(self):
        check_dir()
        status = ssh_utils.scp('veveo123', 'veveo@%s:/data/DATAGEN/DATAGEN_TEST/logs_datagen_test_%s/datagen_test_*.log' % (self.log_ip, CUR_DATE), '/home/veveo/monitoring_mails/datagen_test')
        print 'veveo@%s:/data/DATAGEN/DATAGEN_TEST/logs_datagen_test_%s/datagen_test_*.log' %(self.log_ip, CUR_DATE)
        if status:
            status = ssh_utils.scp('veveo123', 'veveo@%s:/data/DATAGEN/DATAGEN_TEST/logs_datagen_test_%s/datagen_test_*.log' % (self.log_ip, YESTERDAY), '/home/veveo/monitoring_mails/datagen_test')
    ...
    def get_failure_cases(self):
        ...
        for data in open(datagen_file):
            if ('ERROR: GROUP: SEED ID' not in data) and ('ERROR: GROUP: SPORTS ID' not in data):
                continue
            if TE_PATTERN in data:
                ...
                try:
                    ...
                    oth_failures.add((stadium_wiki, got, need, suite_name + ': ' + test_id))
        ...
        if oth_failures:
            text += get_table_data('SPORTS Group Faliure (Total Count: %s)' % (str(len(oth_failures))), ['Gid', 'Got', 'Expected', 'Suite Name: Test ID'], [list(x) for x in set(tuple(x) for x in oth_failures)])

        if to_failures or te_failures or oth_failures:
            self.send_mail(text)
