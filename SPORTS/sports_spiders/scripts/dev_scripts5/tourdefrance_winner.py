...
TOU_RESULTS = "insert ignore into sports_tournaments_results (tournament_id, participant_id, season, result_type, result_sub_type, result_value) values(%s, '0', '2015', '%s', '%s', %s) ON DUPLICATE KEY UPDATE result_type = '%s'"


def main():
    cursor = MySQLdb.connect(host='10.4.18.183', user="root", db="SPORTSDB").cursor()
    query  = "select tournament_id, event_id from sports_tournaments_events where tournament_id in (select id from sports_tournaments where game ='cycling' and type='tournament' and title like '%Tour de France%')"
