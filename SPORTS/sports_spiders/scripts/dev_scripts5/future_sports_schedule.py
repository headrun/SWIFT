...
allowed_sports = ['Golf', 'Formula One', 'Baseball', 'Multi-sport', 'Road cycling', 'American football', 'Basketball', 'Tennis', 'Ice hockey', 'Motorcycle racing', 'Association football', 'Cricket', 'NASCAR', 'Rugby union', 'Track cycling', 'Field hockey', 'Marathon', 'Road bicycle racing', 'Canadian football', 'Horse racing', 'INDYCAR', 'Chess']


class FutureSports(VTVSpider):
    name = 'future_sports'
    start_urls = ['https://en.wikipedia.org/wiki/2015_in_sports']
    ...
    def parse(self, response):
        ...
        for node in nodes:
            ...
            if (self.month_name == event_month) and _date:
                if "–" in _date and not " " in _date:
                    ...
                    event_date = self.construct_date(_date[0])
                if event_date == '':
                    event_date = _date.replace('–', '-')

                sport = extract_data(node, './td[2]/a/text()')
                if sport not in allowed_sports:
                    continue

                event = extract_data(node, './td[3]//text()').replace(u'–', '-')
