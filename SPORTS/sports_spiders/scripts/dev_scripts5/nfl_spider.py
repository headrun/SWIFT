...
class NFLSpiderss(VTVSpider):
    ...
    def get_tz_info(self, loc_city, stadium, game_datetime):
        if loc_city and "Wembley Stadium" in stadium:
            _country = "United Kingdom"
        else:
            _country = "USA"
        tz_info = get_tzinfo(city= loc_city, country = _country, game_datetime = game_datetime)
        if not tz_info:
            tz_info = get_tzinfo(country= _country, game_datetime = game_datetime)
        return tz_info

    @log
    def parse(self, response):
        hxs = Selector(response)
        year = extract_list_data(hxs, '//span[@role="list"]/span[@class="page-nav-label"]/text()')[0]
        top_urls = []
        urls = []
        reg_reference = 'http://www.nfl.com/schedules/%s/REG%s'
        pre_reference = 'http://www.nfl.com/schedules/%s/PRE%s'
        post_reference = 'http://www.nfl.com/schedules/%s/POST'

        for i in range(1, 18):
            top_urls.append(reg_reference % (year, i))

        for i in range(5):
            top_urls.append(pre_reference % (year, i))

        if "/POST" in post_reference:
            top_urls.append(post_reference % year)
        for ref in top_urls:
            if "REG17" in ref:
                year = int(year) + 1
            if self.spider_type == 'schedules':
                yield Request(ref, self.parse_next, meta={'year': str(year)})
            else:
                top_url = 'http://www.nfl.com/scores'
                yield Request(top_url, self.parse_next, meta={'year': str(year)})
    ...
    def parse_next(self, response):
        ...
        if 'schedules'  in self.spider_type:
            ...
            for game_node in game:
                ...
                tou_name = 'National Football League'
                ...
                if match_date:
                    ...
                    if match_time:
                        ...
                        game_datetime = get_utc_time(fdate.strip(), '%A, %B %d %Y', 'US/Eastern')
                ...
                if "Rice" in home_team:
                    home_callsign = ['team_rice']
                    home_team = "Team Rice"
                if "Irvin" in away_team:
                    away_callsign = ['team_irvin']
                    away_team = "Team Irvin"
                ...
                for values in conference:
                    ...
                    if str(away_callsign[0]) == key:
                        conf = value
                print conf
                if not conf and "Divisional Playoffs" in event_info:
                    continue
                if not conf and "Conference Championships" in event_info:
                    continue
 
                if response.url.endswith('/PRE0') and status == 1:
                    event = "Pro Football Hall of Fame Game"
                elif response.url.endswith('/REG1') and status == 1 and \
                game_id == "2015091000/2015/REG1/steelers@patriots":
                ...
                if game_link:
                    ...
                    for key, value in LOC_INFO.iteritems():
                        if str(home_callsign[0]) in key:
                            city, state = value
                            country = "USA"
                    if "Pro-Bowl_TBD_TBD" in game_id :
                        continue
                    city = "Santa Clara"
                    country = "USA"
                    record['source_key']        = game_id
            ...
            for page_url in active_season_games:
                ...
                yield Request(page_url, callback = self.parse_details, \
                        meta={'season_year': season_years[0]})
    ...
    def parse_location_details(self, response):
        ...
        if location:
            loc_city = location[0]
            tz_info = self.get_tz_info(loc_city, stadium_details, record['game_datetime'])
    ...
    def parse_details(self, response):
        ...
        if score_boards:
            for score_board in score_boards:
                ...
                _id       = "".join(re.findall(r'(\d+)/2015', game_id))
                if not _id:
                    _id = game_id.split('/')[0].strip()
    ...
    def parse_next_details(self, response):
        ...
        if "Jan" in date and "2015" not in str(year_) and date:
        ...
        if city:
            tz_info = self.get_tz_info(city, stadium_details[0], game_datetime)
