...
def get_pl_sk(pl_id):
    connection, cursor = mysql_connection()
    query = 'select source_key from sports_source_keys where entity_id =%s and source="usopen_tennis" and entity_type="participant"'
    values = (pl_id)
    cursor.execute(query, values)
    sks = cursor.fetchone()
    if sks:
...
class USOpenTennisSeed(VTVSpider):
    name = "usopen_seed"
    allowed_domains = []
    start_urls = ['https://en.wikipedia.org/wiki/2015_US_Open_(tennis)']
    ...
    def parse(self, response):
        ...
        for node in seed_nodes:
            ...
            if "+" in ptf_def:
                ptf_def = int(ptf_def.split('+')[0]) + int(ptf_def.split('+')[1])
            pts_won = extract_data(node, './/td[6]//text()')
            if "+" in pts_won:
                pts_won = int(pts_won.split('+')[0]) + int(pts_won.split('+')[1])
            pts_after = extract_data(node, './/td[7]//text()')
            group_name = extract_list_data(node, '..//preceding-sibling::h3//span[@class="mw-headline"]//a//text()')[-1]
            ...
            if not group_name:
                group_name = "U.S. Open Women's Singles Seed"
            ...
            record['result']        = {pl_sk: {'rank': rank, 'seed': seed, 'pts_before': pts_before, \
        ...
        for double_seed in double_seeds:
            ...
            count += 1
            if int(count)<=16:
                group_name = "U.S. Open Men's Doubles Seed"
            elif int(count)>16 and int(count)<=32:
                group_name = "U.S. Open Women's Doubles Seed"
            elif int(count)>32 and int(count)<40:
                group_name = "U.S. Open Mixed Doubles Seed"
