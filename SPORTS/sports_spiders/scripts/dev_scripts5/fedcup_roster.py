...
DOMAIN = "http://www.fedcup.com"
class FedcupRoster(VTVSpider):
    name = "fedcup_roster"
    ...
    def parse(self, response):
        ...
        for node in nodes:
            ...
            if "id=GDR" in team_link:
                continue
            elif "id=SUD" in team_link:
                continue
    ...
    def parse_team_info(self, response):
        ...
        for pl_node in player_nodes:
            ...
            if pl_link and year == int(season):
                yield Request(pl_link, callback=self.parse_player_info, meta = {'team_sk': team_sk, 'record': record, 'season': season, \
    ...
    def parse_player_info(self, response):
        ...
        players = {player_sk: {'season': season, 'player_role': role, \
