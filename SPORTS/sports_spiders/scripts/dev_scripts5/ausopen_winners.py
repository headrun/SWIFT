...
TOU_RESULTS = "insert ignore into sports_tournaments_results (tournament_id, participant_id, season, result_type, result_sub_type, result_value) values(%s, '0', '2014', '%s', '%s', %s) ON DUPLICATE KEY UPDATE result_sub_type='%s', result_type = '%s'"


def main():

    cursor = MySQLdb.connect(host='10.4.15.132', user="root", db="SPORTSDB_BKP").cursor()
    query  = " select tournament_id, final_event_id from sports_tournaments_finals where final_event_id in (select id from sports_tournaments where title like '%Australian Open%' and title like '% final%')"
    ...
    for event in data:
        ...
        game_query = 'select id, YEAR(game_datetime), status, game from sports_games where event_id = %s and YEAR(game_datetime) = "2014" and status = "completed"'
        ...
        if event_id :
            ...
            winner_group = str(group_name[0]).lower().replace("'s", '').replace('australian open ', '')
            if len(result_value) == 2 and "doubles" in winner_group:
                ...
                cursor.execute(TOU_RESULTS %(15, 'winner', winner_group.lower(), result_value[0][0], winner_group.lower(), 'winner'))
                cursor.execute(TOU_RESULTS %(15, 'winner2', winner_group.lower(), result_value[1][0], winner_group.lower(), 'winner2'))
            else:
                cursor.execute(TOU_RESULTS %(tou_id, 'winner', winner_group.lower(), result_value[0][0], winner_group.lower(), 'winner'))
                cursor.execute(TOU_RESULTS %(event_id, 'winner', winner_group.lower(), result_value[0][0], winner_group.lower(), 'winner'))
                cursor.execute(TOU_RESULTS %(final_event_id, 'winner', winner_group.lower(), result_value[0][0], winner_group.lower(), 'winner'))
