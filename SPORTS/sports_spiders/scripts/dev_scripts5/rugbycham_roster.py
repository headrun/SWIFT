...
class RugbychampRoster(VTVSpider):
    name = "rugbychamp_roster"
    allowed_domains = []
    start_urls = ['http://www.sanzarrugby.com/therugbychampionship/teams/']
    participants = {}
    ...
    def parse(self, response):
        ...
        for tm_node in tm_nodes:
            team_url = extract_data(tm_node, './/@href')
            print team_url
            season = team_url.split('=')[2].split('&')[0].strip()
            url = "http://omo.akamai.opta.net/competition.php?feed_type=ru4&competition=214&season_id=%s&user=owv2&psw=wacRUs5U&jsoncallback=RU4_205_%s_t73" % (season, season)
    ...
    def parse_next(self, response):
        ...
        repl_data = response.url.split('=')[-1].strip()
        raw_data    = response.body.replace(repl_data, '').replace(')', '').replace('(', '')
        data        = eval(raw_data)
