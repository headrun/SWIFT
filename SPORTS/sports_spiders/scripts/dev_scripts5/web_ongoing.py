...
REPLACE_WORDS_by = {"hawai'i": 'hawaii', 'the senior': 'senior', 'hickory kia classic': 'hickory classic'}
...
class WebcomOngoing(VTVSpider):
    ...
    def parse_start_nodes(self, response):
        ...
        for node in nodes:
            ...
            if tournament_link and 'http' not in tournament_link:
                tournament_link = "http://www.pgatour.com" + tournament_link
            ...
            details = {'str_date' : str_date, 'e_date' : e_date, 'loc' : loc, 'tou_name' : tou_name, 'stadium': stadium, \
                        'tou_date' : tou_date, 'def_chp' : defending_chp, 'prize_money' : prize_money, \
                        'start_date' : start_date, 'end_date' : end_date, 'tou_year' : tou_year,  'tz_info': tz_info, \
                        'game_status' : status, 'tour_name' : tour_name, 'tournament_link' : tournament_link, 'game_datetime' : game_datetime}


            if status == "ongoing" :
                yield Request(tournament_link, callback = self.parse_listing, meta = {'details' : details})
            if status == "completed" and end_date == new_date:
                yield Request(tournament_link, callback = self.parse_listing, meta = {'details' : details})
    ...
    def parse_listing(self, response):
        ...
        tou_image = extract_data(hxs, '//div[@class="banner"]//img/@src')
        if tou_image and 'http' not in tou_image:
            tou_image = get_full_url(tou_image, response.url)
        elif not tou_image:
            tou_image = 'http://www.pgatour.com/etc/designs/pgatour/element/img/5.0/global/nav/web_com_logo.png'
        ...
        if leader_board_link and 'http' not in leader_board_link:
            leader_board_link = "http://www.pgatour.com" + leader_board_link
        ...
        if result_link and 'http' not in result_link:
            result_link = "http://www.pgatour.com" + result_link

        details.update({'result_link' : result_link, 'leader_board_link' : leader_board_link})
        ...
        if not details['tou_date'] or not details['tou_name']:
        ...
        elif leader_board_link and ('www.europeantour.com' in leader_board_link or 'www.pgatour.com' not in leader_board_link):
            print "Not PGA Site, Tou Name: %s ---- Date: %s ---- Link: %s" % (details['tou_name'], details['tou_date'], leader_board_link)
        else:
            ...
            elif details['game_status'] == "ongoing":
    ...
    def parse_results(self, response):
        ...
        if target_par and str(details['end_date'].year) in tour_ending and total_rounds:
            ...
            for node in player_nodes[1:]:
                #player_data = node.xpath('.//td/text()')
                ...
                if not player_name or (len(set(rounds_scores)) == 1 and rounds_scores[0] == 0 and not is_score_zero) or not position:
                    continue

                player_id = self.player_id_sk.get(player_name, '')
