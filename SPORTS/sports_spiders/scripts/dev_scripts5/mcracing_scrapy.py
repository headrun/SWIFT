...
from vtvspider_new import VTVSpider, get_nodes, extract_data, log, \
...
def get_event_id(tou_name, event_name_rac):
    ...
    for key, value in REPLACE_EVENT_DICT.iteritems():
        if key.upper() in X(tou_name).upper():
            event_name_db = value
            event_name_db = event_name_db.replace('motorcycle', event_name_rac).replace(' Community', '').replace("and Rimini's Coast ", '').strip()
    ...
    def parse_result(self, response):
        ...
        if "RAC" in result_url:
            ...
            for ind, row in enumerate(rows):
                ...
                if (data != "Not Classified")  and (data != "Not Starting") and (data != "Not Finished 1st Lap"):
                    if count < 9:
