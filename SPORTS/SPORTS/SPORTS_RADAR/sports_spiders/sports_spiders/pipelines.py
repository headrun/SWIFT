...
from vtv_utils import VTV_SERVER_DIR
...
class SportsGames(object):
    ...
    def spider_opened(self, spider):
        ...
        self.conn         = MySQLdb.connect(db=self.hash_conf['DB_NAME'],
                                            ...
                                            charset="utf8", passwd = 'veveo123', 
                                            use_unicode=True)
        self.cursor       = self.conn.cursor()
        self.spider_class = spider.__class__.__name__
        self.gids_file = os.path.join(STATS_DIR, self.spider_class + '_gids.pickle')
        log_name  = os.path.join(STATS_DIR, self.spider_name)
        self.items_log = open(log_name, 'ab+')
    ...
    def write_log(self, item):
        ...
        #self.items_log.write(str(datetime.now()) + ': ' + str(items_dict) + '\n\n')

    def process_item(self, item, spider):
        self.write_log(item)
        sports_item = gu.SportsdbSetup(item, self.cursor, self.spider_class, self.gids_file, self.hash_conf)
