...
RIGHT_QUERY = 'insert into sports_image_rights_mapping(image_id, right_id, created_at, modified_at) values(%s, 1, now(), now())'
...
class GettyNewcrawler(VTVSpider):
    ...
    def start_requests(self):
        ...
        self.headers.update({'authorization': bearer_key})
        ...
        for each in data:
            event_id = str(each[0])
            league = each[1]
            url = 'https://api.gettyimages.com/v3/search/images?event_ids=%s&page_size=100&page=%s&phrase=%s' % (event_id, 1, league)
            yield Request(url, callback=self.parse_images, headers=self.headers, meta={'event_id':event_id, 'league':league})
    ...
    def parse_images(self, response):  
        ...
        yield Request(images, callback=self.parse_images_all, headers=self.headers, meta={'league':league, 'event_id':event_id})
    ...
    def parse_images_all(self, response):
        ...
        if response.status == 401 :
            ...
            if not response.meta.get('counter', ''):
                yield Request(images, callback=self.parse_images_all, headers=self.headers, meta={'league':league, 'event_id':event_id, 'counter':1})
