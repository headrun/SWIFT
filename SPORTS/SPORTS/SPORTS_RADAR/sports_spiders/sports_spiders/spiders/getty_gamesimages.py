...
RD_IMAGE_QUERY = 'insert into sports_images (url_sk, image_url, image_type, height, width, description, image_created, image_updated, league, created_at, modified_at) values (%s, %s, %s, %s, %s, %s, %s, %s, %s , now(), now()) on duplicate key update modified_at=now(), image_type= %s'


class GettyGamesImages(VTVSpider):
    name = "getty_gamesimages"
    ...
    def start_requests(self):
        ...
        yield FormRequest(url.replace('https', 'http'), callback=self.parse_login, formdata=_data_dict, meta={'proxy': 'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
    ...
    def parse_login(self, response):
        ...
        for each in data:
            ...
            url = url%values
            yield Request(url.replace('https', 'http'), callback=self.parse_next, meta = {'event_id': event, 'league': each[1], 'proxy': 'http://game-dynamics-proxy-dev-0-489291303.us-east-1.elb.amazonaws.com:8080'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
    ...
    def parse_next(self, response):
        ...
        for node in pic_nodes:
            ...
            yield Request(lic_url.replace('https', 'http'), callback=self.parse_image, meta = {'league': league, 'event_id':response.meta['event_id'], 'pic_url':pic_url, 'league': each[1], 'proxy': 'http://game-dynamics-proxy-dev-0-489291303.us-east-1.elb.amazonaws.com:8080'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
        ...
        if cur_page and max_page and int(cur_page)<=int(max_page):
            ...
            yield Request(ur.replace('https', 'http'), self.parse_next, meta = {'event_id': response.meta['event_id'], 'league': response.meta['league'], 'proxy': 'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080/'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
        self.cursor.close()
        self.conn.close()
