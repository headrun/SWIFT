...
class HeadshotsV3Images(VTVSpider):
    ...
    url = 'http://api.sportradar.us/%s-images-p%s/usat/headshots/players/%s/manifest.xml?api_key=%s'
    ...
    def start_requests(self):
        for key, values in self.apis_dict.iteritems():
            ...
            yield Request(req_url.replace('https', 'http'), self.parse, meta={'game': key, 'version': version, 'api-key': api_key, 'proxy': 'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080/'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
    ...
    def parse(self, response):
        ...
        for node in nodes:
            ...
            player_id = self.get_player_id(player_sk, cursor)
            if player_id:
                self.populate_images(node, player_id, game, version, api_key, cursor)
            else:
                name = extract_data(node, './/title/text()')
                self.get_player_details(name, player_sk, node, game, version, api_key, cursor)

        cursor.close()
        conn.close()

    def populate_images(self, node, player_id, game, version, api_key, cursor):
        ...
        for image_node in images:
            ...
            if count:
                ...
                cursor.execute("select auto_increment from information_schema.TABLES where TABLE_NAME='sports_images' and TABLE_SCHEMA='SPORTSDB'")
            ...
            cursor.execute(SP_IMG_RIGHT_ID, right_values)

            if player_id:
                values = (player_id, 'player', count)
                cursor.execute(IMG_MAP_QUERY, values)
            else:
                self.images_file.write('%s<>%s<>%s<>%s\n' % (player_name, image_sk, game, player_sk))

    def get_player_details(self, name, player_sk, node, game, version, api_key, cursor):
        pl_id = ''
        if game == 'nfl':
            key_game = '4'
        elif game == 'mlb':
        ...
        cursor.execute(query, (key_game, name))
        data = cursor.fetchall()
        data = [str(dt[0]) for dt in data]
        if data and len(data) == 1:
            values = (data[0], 'participant', 'radar', player_sk)
            cursor.execute(INSERT_SK, values)
            pl_id = data[0]
        if data:
            rec = '<>'.join(data)
            record = '%s<>%s<>%s' % (rec, name, player_sk)
            self.pl_exists.write('%s\n' % record)
        self.populate_images(node, pl_id, game, version, api_key, cursor)


    def get_player_id(self, sk, cursor):
        cursor.execute(SK_CHECK , ('radar', 'participant', sk))
