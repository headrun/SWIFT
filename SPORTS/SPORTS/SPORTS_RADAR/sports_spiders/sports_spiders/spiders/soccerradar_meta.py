...
INT_PL = 'insert into sports_players(id, title, sport_id, first_name, last_name, abbr_name, reference_url, debut, country, country_code, main_role, roles, height, weight, birth_date,birth_place, college, high_school, experience, salary, updated, full_first_name, full_last_name, created_at, modified_at) values (%s, %s, %s, %s, %s, %s, %s,%s, %s, %s,%s, %s, %s, %s,%s, %s,%s, %s, %s, %s, %s, %s, %s, now(), now()) on duplicate key update modified_at = now(), sport_id = %s, full_first_name = %s, full_last_name = %s'
...
TEAM_URL =  'http://api.sportradar.us/soccer-p2/%s/teams/%s/profile.xml?api_key=%s'

class SoccerRadarMeta(VTVSpider):
    name       = "soccer_radar_meta"
    start_urls = ["http://api.sportradar.us/soccer-p2/eu/teams/hierarchy.xml?api_key=72vxpdus7zy22a93kcnhjqct", \
                "http://api.sportradar.us/soccer-p2/na/teams/hierarchy.xml?api_key=kpwbhzaxkpxhfgp3zdaqe2xw"]
        ...
        for tou_node in tou_nodes:
            ...
            for group_node in group_nodes:
                ...
                for node in team_nodes:
                    ...
                    yield Request(team_url.replace('https','http'), self.parse_team, meta = {'tou_id': tou_grp_id, 'season': tou_season, 
                    'proxy': 'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
    ...
    def parse_team(self, response):
        ...
        manager_full_first_name = extract_data(sel, '//manager/@full_first_name')
        manager_full_last_name = extract_data(sel, '//manager/@full_last_name')
        ...
        pl_values = (manager_id, manager_fullname, sport_id, manager_firstname, manager_lastname, \
                    ...
                    college, high_school, experience, salary, updated, manager_full_first_name, \
                    manager_full_last_name, sport_id, manager_full_first_name, manager_full_last_name)
        self.cursor.execute(INT_PL, pl_values)

        roster_values = (team_id, manager_id, 'Manager', '', '', response.meta['season'], response.meta['season'])
        ...
        for each in nodes:
            ...
            pl_full_first_name = extract_data(each, './@full_first_name')
            pl_full_last_name  = extract_data(each, './@full_last_name')
            status             =   ''
            pl_abbr_name       =  pl_pr_position    =   college = \
            high_school        = experience         =   salary = \
            birth_place        =   ''
            ...
            pl_values = (pl_id, pl_name, sport_id, pl_first_name, pl_last_name, \
                        ...
                        college, high_school, experience, salary, updated, pl_full_first_name, \
                        pl_full_last_name, sport_id, pl_full_first_name, pl_full_last_name)
            self.cursor.execute(INT_PL, pl_values)
