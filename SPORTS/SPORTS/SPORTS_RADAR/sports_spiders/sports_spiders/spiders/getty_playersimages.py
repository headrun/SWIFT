...
class GettyPlayersImages(VTVSpider):
    ...
    def parse_login(self, response):
        ...
        for each in data:
            ...
            url = url%values
            yield Request(url.replace('https', 'http'), callback=self.parse_next, meta = {'event_id': event, 'league': each[1], 'proxy': 'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080/'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
        self.cursor.close()
        self.conn.close()
    ...
    def parse_next(self, response):
        ...
        for node in pic_nodes:
            ...
            yield Request(lic_url.replace('https', 'http'), callback=self.parse_image, meta = {'league': league, 'event_id':response.meta['event_id'], 'pic_url':pic_url, 'proxy': 'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080/'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
        ...
        if cur_page and max_page and int(cur_page)<=int(max_page):
            ...
            yield Request(url.replace('https', 'http'), self.parse_next, meta = {'event_id': response.meta['event_id'], 'league': response.meta['league'], 'proxy': 'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080/'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
