...
class SoccerAPIImages(VTVSpider):
    ...
    #apis_dict = {'f1': {'api-key': 'x7qwcj8mkz76xv4hu624murn', 'version': '3', 'sport': 'f1', 'provider': 'reuters'}}

    sport_dict = {'cricket': 6, 'tennis': 5, 'rugby union': 11, 'f1': 10, 'auto racing': 10}
    ...
    def start_requests(self):
       ...
       for i in range(0, 30):
            ...
            for key, values in self.apis_dict.iteritems():
                ...
                yield Request(req_url.replace('http', 'http'), self.parse,  \
...
#              self.f.write(title.encode('utf-8') + image_link[0]+'\n')
    ...
    def get_plid(self,title,key):
        ...
        for player in player_bigrams:
            ...
            player = " ".join(player)
            player = '%'+player+'%'
            query ="select id,title from sports_participants where participant_type='player' and title like %s and sport_id=%s"
            ...
            if not result:
                ...
                query ="select id,title from sports_participants where participant_type='player' and aka like %s and sport_id=%s"
                values = (player.replace(',','').replace('\'',''),key)
                self.cursor.execute(query,values)
                result = self.cursor.fetchall()
