...
<html>
<head>
    <title> Mashup Related Movies </title>
</head>
<body>
    <br />
    %s
...
def print_suggestions(simmovie, search_string, suggestions, port):
    ...
    for gid, title in suggestions:
        ...
        print '<tr bgcolor="wheat" ><td>%s</td><td><a href="simmovie_client.py?port=%s&action=related+%s">%s</a></td></tr>'%(gid,port,gid,title)
    print '</table>'

def print_related(simmovie, guid, related, port):
    cur_rec = simmovie.getGidInfo(guid, 'metaInfo')
    PROG_SCHEMA = simmovie.PROG_SCHEMA
    ...
    if not related:
        ...
        return	
    str="<div> Related Movies for: %s:<a href='simmovie_client.py?port=%s&action=%s'>%s</a><br><b>Gk:</b> %s<br><b>Ke:</b> %s<br><b>Gg:</b> %s<br><b>Ge:</b> %s<br><b>Su:</b> %s<br><b>Cl:</b> %s<br><b>Ry:</b> %s<br><b>Ll:</b> %s<br><b>Ra:</b> %s</div>\n"%(guid, port, "related+"+guid, title, gkwds, kwds, good_genres, genres, super_genres, country, ryear, language, rating)
    print html%str
	...
	meta_rec = simmovie.getGidInfo(gid, 'metaInfo')
	...
	if cur_kwds:
        ...
        print '<tr bgcolor="wheat" ><td>%s</td><td nowrap><a href="simmovie_client.py?port=%s&action=related+%s">%s</a></td><td><no_wrap="">%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>'%(num,port, gid, gid, cur_title, good_cur_kwds, cur_kwds, cur_good_genres, cur_genres, cur_super_genres, cur_country, cur_ryear, cur_lang, cur_rating, final_score, debug_string)
    print '</table>'
...
def mashup_engine():
    ...
    print
    logger = initialize_logger("/tmp/simmovie_client.log")
	...
	action_id = input
    simmovie = SimilarMovie(logger, port)
   
    if action_id == 'related':
	related = simmovie.getGidInfo(arguments, 'related')
	print_related(simmovie, arguments, related, port)
    elif action_id == 'search':
        suggestions = simmovie.searchRecords(arguments)
	print_suggestions(simmovie, arguments, suggestions, port) 
    else:
	print html%"Specify 'related-string-query' as http://%s/tools/simmovie_client.py?port=%s&action=related+gid\n"%(simmovie.ip_address, port)
	str = "i.e. To search Movie containing string 'lord' query should be\n"
	print html%str
        str = "<div><a href='simmovie_client.py?port=%s&action=search+lord'> http://%s/simmovie_client.py?port=%s&action=search+lord</a></div>\n"%(port,simmovie.ip_address, port)
	print html%str
