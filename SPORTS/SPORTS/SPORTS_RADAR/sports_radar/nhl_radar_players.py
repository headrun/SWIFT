...
PL_RADAR = 'insert into sports_radar_players(participant_id, source, source_key, title, aka, game, sport_id, height, weight, participant_type, position, college, school, participant_style, birth_place, birth_date, debut, jersey_number, created_at, modified_at) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, now(), now())on duplicate key update modified_at = now()'
...
class NHLRadarJson(VTVSpider):
    ...
    def parse(self, response):
	    ...
	    if pl_id:
		sports_id = get_sport_id("hockey")
		pl_values = (pl_id, "radar", radar_sk, full_name, "", "hockey", sports_id, height, weight, "player", position, college, school, "", birth_place, birthdate, pro_debut, jersey_number)
