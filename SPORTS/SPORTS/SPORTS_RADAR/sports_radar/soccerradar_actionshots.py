...
TEAMS_DICT = {'Carpi': "Carpi FC 1909", "Juventus": "Juventus F.C.", "Atalanta": "Atalanta B.C.", \
		...
		"GFCO Ajaccio": "Ajaccio GFCO", "Bordeaux": "FC Girondins de Bordeaux", "Empoli": "Empoli", \
...
class SoccerAPIActionImages(VTVSpider):
	...
	apis_dict = {'bundesliga': {'api-key': 'pr253kju82fm9964ypgk6zg2', 'version': '3', 'sport': 'soccer', 'provider': 'reuters'},
	...
	def parse(self, response):
		...
		for node in nodes:
			...
			for team in teamnames:
				...
				query  =  "select id from sports_participants where title ='%s' and game='soccer' and participant_type='team'"%(team)
				...
				if res_query:
					...
					if players_id:
						for each_id in players_id:
							query  =  "select id, title from sports_participants where participant_type='player' and id='%s' and game='soccer'" %each_id[0]
							self.cursor.execute(query)
