...
class GettyNewcrawler(VTVSpider):
    ...
    def start_requests(self):
        ...
        for each in data:
            ...
            yield Request(url, callback=self.parse_images, headers=headers, meta={'headers':headers, 'player_id':player_id, 'league':league})
    ...
    def parse_images(self, response):  
        ...
        print images
        yield Request(images, callback=self.parse_images_all, headers=headers, meta={'headers':headers, 'league':league, 'player_id':player_id})
    ...
    def parse_images_all(self, response):
        ...
        if response.status == 401 :
            ...
            if not response.meta.get('counter', ''):
                yield Request(images, callback=self.parse_images_all, headers=headers, meta={'league':league, 'player_id':player_id, 'headers':headers, 'counter':1})
