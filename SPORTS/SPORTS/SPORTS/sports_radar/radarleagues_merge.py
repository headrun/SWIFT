...
class SportsRadarMerge(VtvTask):
    ...
    def get_db_teams(self):
        query = 'select id, title from sports_participants where participant_type = "team" and sport_id=4'
        self.cursor.execute(query)
        players_data = self.cursor.fetchall()
        for player_data in players_data:
            id_, title = player_data
    ...
    def ncaa_teams(self):
        ...
        for tm_data in data:
            ...
            tm_title = tm_title.replace('Central Connecticut State', 'Central Connecticut').replace('Lehigh Hawks', 'Lehigh Mountain Hawks').replace('Virginia-Wise', 'Virginia Wise').replace('Charleston (WV)', 'Charleston Univ').replace('Minnesota State Moorhead', 'Minnesota State Univ.-Moorhead').replace('Gustavus', 'Gustavus Adolphus Golden').replace('St. Johns Johnnies', "St. John's (Minn.) Johnnies").replace('St. Thomas Tommies', "St. Thomas (Minn.) Tommies")
            ...
            if not data:
                tm_title =tm_title.replace(' College', '').replace(' University', '').strip().replace(' Univ ', ' Univ. ')
                values = (tm_title, sport_id)
                self.cursor.execute(sp_qry, values)
                data = self.cursor.fetchone()
