...
class CricketRadar(VTVSpider):
    ...
    def __init__(self):
        ...
        self.cricket_exists_file = open('cricket_exists_file', 'w')
        self.cricket_missed_file = open('cricket_missed_file', 'w')
    ...
    def parse_teams(self, response):
        ...
        for node in player_nodes:
            ...
            player_nick = ''.join(node.xpath('./@nick_name').extract())
    ...
    def get_plid(self, site_birt_date, full_name, player_nick):
        qry = 'select P.id from sports_participants P, sports_players PL where P.title="%s" and P.game="cricket" and P.id=PL.participant_id and PL.birth_date="%s"'
        self.cursor.execute(qry %(full_name, site_birt_date))
        data = self.cursor.fetchone()
        if data:
            pl_id = data[0]
        else:
            qry ='select P.id from sports_participants P, sports_players PL where P.title like "%s" and P.game="cricket" and P.id=PL.participant_id and PL.birth_date="%s"'
            self.cursor.execute(qry %('%'+player_nick+'%', site_birt_date))
            data = self.cursor.fetchone()
            if data:
                pl_id = data[0]
            else: pl_id = ''
    ...
    def get_db_players(self, team):
        query = 'select id, title from sports_participants where title like "%s" and title not like "%s"' %('%'+team+'%'+'cricket'+'%', '%'+'women'+'%')
        try:
            self.cursor.execute(query)          
        except:
            print 'select id, title from sports_participants where title like %s' %('%'+team+'%'+'national cricket'+'%')
        tou_id = self.cursor.fetchall()
        if tou_id:
            tou_id = tou_id[0][0]
        else:
            print "No Tournament foudn ..."
            print '%'+team+'%'+'national cricket'+'%'
            return
