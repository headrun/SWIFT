    ...
    def get_team_details(self, teams, game, node, version, api_key, title, league):
        ...
        for team in teams:
          if "ncaafb" in league:
              ...
              pa_query = 'select id from sports_participants where title=%s and sport_id =%s'
              self.cursor.execute(pa_query, (par, game_))
              data = self.cursor.fetchall()
              if not data and game_ == "2":
                  pa_query = 'select id from sports_participants where title like %s and sport_id=%s and title not like %s and participant_type="team"'
                  par = "%" + par + "%"
