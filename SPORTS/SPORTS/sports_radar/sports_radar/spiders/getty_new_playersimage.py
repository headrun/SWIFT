...
class GettysNewcrawler(VTVSpider):
    ...
    start_urls = []
    handle_httpstatus_list = [401,400,404]

    def __init__(self, *args, **kwargs):
        super(GettysNewcrawler, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        league ={'national football league': 'National Football League','premier league':'Premier League','chile primera division':'Chile Primera Division','argentina primera division':'Argentina Primera Division','campeonato brasilerio':'Campeonato Brasileiro Serie A','la liga':'La Liga','liga mx':'Liga MX','ligue 1':'Ligue 1','bundesliga':'Bundesliga','serie A':'Serie A'}
	lea = league[self.spider_type]
	values = (lea)
	self.cursor.execute(PLAYERS_QUERY  % values) 
    ...
    def parse_images_all(self, response):
        ...
        for each_image in images:
            ...
            keywords_details = str("<>".join(keywords)).encode('utf-8').strip()
            weight_count =0
            ...
            try:
                ...
                epopch_time = 0
            created_data = each_image.get('date_created', '0000-00-00')
            keywords_repeating = [each['type'] for each in each_image['keywords']]
            count_of_player = keywords_repeating.count('SpecificPeople')
            print count_of_player
            print keywords
