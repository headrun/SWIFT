...
class RadarSoccer(VTVSpider):
    ...
    def populate_team_sk(self, name, radar_sk):
        query = 'select id from sports_participants where sport_id="7" and gid like "%TEAM%" and title like %s'
        t_name = '%' + name + '%'
    ...
    def start_requests(self):
        for league in ['eu']:#['eu', 'na']:
            ...
            yield Request(req_url.replace('https', 'http'), self.parse, meta = {'league': league, 'api_key': api_key, 'proxy':'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080/'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
    ...
    def parse(self, response):
        ...
        for api_key in self.team_apis:
            ...
            yield Request(req_url.replace('https', 'http'), self.parse_next, meta={'league': league, 'proxy':'http://internal-sports-api-proxy-prod-0-1192592570.us-east-1.elb.amazonaws.com:8080/'}, headers = {"X-SPORTSAPI-EXT-PROXY-CLIENT": "Sports_KG"})
    ...
    def add_player_sk(self, name, birthdate, radar_sk):
        query = 'select sp.id from sports_participants sp, sports_players pl where sport_id="7" and sp.id=pl.participant_id and title =%s and birth_date=%s'
