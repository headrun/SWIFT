...
IMAGE_QUERY = 'insert into sports_images (url_sk, image_url, image_type, height, width, description, image_created, image_updated, league, created_at, modified_at) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, now(), now()) on duplicate key update modified_at=now(), league = %s'
...
#    apis_dict = {'ligue-1': {'api-key': '8y9a9gjspp8bcykxnn4cj2wk', 'version': '3', 'sport': 'soccer', 'provider': 'reuters'}}
    ...
    def populate_player_id(self):
        ...
        query = "select player_id, team_id from sports_roster where status='active' and season in ('2016-17')"
    ...
    def parse(self, response):
        ...
        for node in nodes:
            ...
            if player_id:
                self.populate_images(node, player_id, sport, version, provider, api_key, game)
            else:
                name = extract_data(node, './/title/text()')
                self.get_player_details(name, player_sk, node, version, sport, api_key)

    def populate_images(self, node, player_id, sport, version, provider, api_key, game):
        ...
        for image_node in images:
            ...
            values = (image_sk, image, image_type, height, width, description, image_created, image_updated, game, game)
            ...
            if player_id:
                ...
                if team_id:
                    ...
                    self.cursor.execute(IMG_MAP_QUERY, values)
                    #query = "update sports_images_mapping set entity_id = %s where image_id = %s  and entity_type = 'team' limit 1"
                    # query % (team_id, count)
    ...
    def get_player_details(self, name, player_sk, node, version, sport, api_key):
        ...
        self.cursor.execute(query, ('7', name))
