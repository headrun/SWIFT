...
class WeddingWire(scrapy.Spider):
	name = "weddingWireBrowser"
	...
	def parse(self,response):
		headers = {
			...
			'referer': 'https://www.weddingwire.ca/wedding-vendors/ontario/ottawa',
			'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8'
		}
		res = Selector(response)
		locations_urls = res.xpath('//li[contains(@data-name,"id_poblacion")]')
		for locations in locations_urls:
			vendors_count = ''.join(locations.xpath('./span[contains(@class,"vendorsFilters__count")]/text()').extract()).strip()
			if vendors_count != '':
				vendors_count_number = int(vendors_count.replace(',',''))
				if vendors_count_number >= 5:
					data_url = 'https://www.weddingwire.ca/wedding-vendors/'+''.join(locations.xpath('@data-url').extract())
					meta = {'dataUrl': data_url, 'page': 1, 'totalPage': math.ceil(vendors_count_number/14)}
					yield Request(data_url,headers=headers,callback=self.parse1, meta=meta)
		# # data_url = 'https://www.weddingwire.ca/weddings/ontario/toronto'
		# data_url = 'https://www.weddingwire.ca/wedding-photography/british-columbia/vancouver'
		# meta = {'dataUrl': data_url, 'page': 1, 'totalPage': math.ceil(226/14)}
		# yield Request(data_url,headers=headers,callback=self.parse1, meta=meta)
	...
	def parse2(self,response):
		...
		final_sub_dict["Description"] = description
