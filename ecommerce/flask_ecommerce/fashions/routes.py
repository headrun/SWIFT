...
def brand():
    ...
    if source == "all":
        if category == "all":
            sql_query = "select brand from products_info group by brand;"
        else:
            sql_query = "select brand from products_info where category='%s' group by brand;"%(category)
    else:
        if category == "all":
            sql_query = "select brand from products_info where source='%s' group by brand;"%(source)
        else:
            sql_query = "select brand from products_info where source='%s' and category='%s' group by brand;"%(source,category)
    brandsData = pd.read_sql(sql_query,con=db_connection)
    return brandsData.to_json(orient='records')
@app.route('/sources')
def source():
    db_connection_str = 'mysql+pymysql://root:Ecomm@34^$@127.0.0.1/ECOMMERCEDB'
    db_connection = create_engine(db_connection_str)
    sourceData = pd.read_sql("select source from products_info group by source;",con=db_connection)
    return sourceData.to_json(orient='records')
@app.route('/search')
def data():
    start_date = request.args.get('startdate')
    end_date = request.args.get('enddate')
    ...
    queryData = pd.read_sql(sql_query,con=db_connection)
    if totalCount != 0:
        return {"totalCount":totalCount,"products":queryData.to_json(orient='records')}
    else:
        return {"products":queryData.to_json(orient='records')}
