...
from ecommerce.common_utils import EcommSpider,\
    extract_list_data, extract_data, encode_md5, \
    normalize
...
class CostcoSpider(EcommSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.category_array = ['mens-outerwear.html', 'mens-activewear.html', 'pants.html', 'mens-shirts.html',
                               'mens-sweaters.html', 'mens-suits.html', 'mens-sleepwear.html', 'mens-shorts.html', 'mens-swimwear.html']
        self.headers = {
    ...
    def parse(self, response):
        ...
        reviews = extract_list_data(
            sel, '//meta[@itemprop="reviewCount"]/@content')
        ratings = extract_list_data(
            sel, '//meta[@itemprop="ratingValue"]//@content')
        availabilities = extract_list_data(
            sel, '//input[@id="in_Stock"]//@value')
        for url, review, rating, availability in zip(urls, reviews, ratings, availabilities):
            meta = {'category': response.url, 'review': review,
    ...
    def parse_data(self, response):
        ...
        if response.meta['availability']:
            availability = 1
        brandname = normalize(extract_data(sel, '//div[@itemprop="brand"]//text()'))
        product_name = normalize(extract_data(sel, '//h1[@itemprop="name"]//text()'))
        description = normalize(extract_data(sel, '//ul[@class="pdp-features"]//li//text()'))
        currency_ = normalize(extract_data(sel, '//meta[@property="product:price:currency"]//@content'))
        mrp = normalize(extract_data(sel, '//meta[@property="product:price:amount"]//@content'))
        try:
            category_ = normalize(category.split('/')[-1].split('-')[0].replace('s', ''))
            sub_category = normalize(category.split('/')[-1].split('-')[1].replace('.html', ''))

        except IndexError as error:
            self.log.error('Error while fetching the category - %s', error)
            category_ = 'men'
        ...
        image_url = normalize(extract_data(sel, '//img[@class="img-responsive"]//@src'))
        sizes = normalize(extract_list_data(sel, '//select[@class="varis form-control"]//option[contains(@data-attr-name, "Size")]//text()'))
        skus = normalize(extract_list_data(sel, '//select[@class="varis form-control"]//option[contains(@data-attr-name, "Size")]//@value'))
